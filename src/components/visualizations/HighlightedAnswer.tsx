  ...
  return (
    <div className="flex flex-col gap-2">
      {contextText && start !== undefined && end !== undefined ? (
        ...
        <div className="text-sm font-mono text-[var(--relax-accent)] font-bold">
          {answer}
        </div>
      )}
      {score !== undefined && (
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-[var(--relax-text-muted)] font-bold tracking-widest">
            CONFIDENCE
          </span>
          <span className="text-[10px] font-mono text-[var(--relax-success)] font-bold">
            {(score * 100).toFixed(1)}%
          </span>
        </div>
      )}
