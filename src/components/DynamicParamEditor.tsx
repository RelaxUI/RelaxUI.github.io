        ...
        <div key={key} className="flex flex-col gap-0.5">
          ...
          {def.type === "number" && (
            <input
              ...
              className="w-full bg-[#0b0e14]/60 border border-[#1f2630] rounded px-2 py-1 text-xs font-mono text-white focus:outline-none focus:border-[#00e5ff] nowheel nodrag"
            />
          )}
          {def.type === "boolean" && (
            <button
              ...
              className={`relative w-8 h-4 rounded-full transition-colors shrink-0 ${(values[key] ?? def.default) ? "bg-[#00e5ff]" : "bg-[#1f2630]"}`}
            >
              <div
          ...
          {def.type === "dropdown" && def.options && (
            <select
              ...
              className="w-full bg-[#0b0e14]/60 border border-[#1f2630] rounded px-2 py-1 text-xs font-mono text-white focus:outline-none focus:border-[#00e5ff] nodrag"
            >
              {def.options.map((opt) => (
